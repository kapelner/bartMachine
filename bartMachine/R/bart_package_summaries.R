...
summary.additiveBartMachine = function(object, ...){	
	first_bartmachine = object$models[[1]]
	cat(paste("Additive BART model build via bartMachine v", packageVersion("bartMachine"), ifelse(first_bartmachine$pred_type == "regression", " for regression", " for classification"), "\n\n", sep = ""))
	if (first_bartmachine$use_missing_data){
		cat("Missing data feature ON\n")
	}
	#first print out characteristics of the training data
	cat(paste0("number of specified covariate interactivity constraints = ", length(object$original_constraints), "and num runs through = ", object$num_runs_through, ".\ntraining data size: n = ", first_bartmachine$n, " and p = ", object$p, "\n"))
	...
	if (ttb > 60){
		ttb = as.numeric(object$time_to_build, units = "mins")
		cat(paste("built in", round(ttb, 2), "mins on", first_bartmachine$num_cores, ifelse(first_bartmachine$num_cores == 1, "core,", "cores,"), first_bartmachine$num_trees, "trees,", first_bartmachine$num_burn_in, "burn-in and", first_bartmachine$num_iterations_after_burn_in, "post. samples\n"))
	} else {
		cat(paste("built in", round(ttb, 1), "secs on", first_bartmachine$num_cores, ifelse(first_bartmachine$num_cores == 1, "core,", "cores,"), first_bartmachine$num_trees, "trees,", first_bartmachine$num_burn_in, "burn-in and", first_bartmachine$num_iterations_after_burn_in, "post. samples\n"))
	}
...
print.additiveBartMachine = function(x, ...){summary(x)}
